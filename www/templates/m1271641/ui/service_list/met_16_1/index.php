...
<div class="$uicss met-index-body text-xs-center <if value="$ui['bg_type'] eq 1">bgcolor<else/>bgpic</if> <if value="$ui[ifdisplay]&&!$_M['form']['pageset']">conceal<else/>display</if>" m-id='{$ui.mid}'>
	<div class="<if value='$ui[ifwidth]'>container<else/>container-fluid</if>">
		<if value="$ui['title']">
			<h2 class="<if value='$ui[news_subtitle]'>teshu</if> m-t-0 invisible" data-plugin="appear" data-animate="slide-top" data-repeat="false">{$ui.title}</h2>
		</if>
		 <if value="$ui['news_subtitle']">
        <p class="subtitle m-b-10 " >{$ui.news_subtitle}</p>
        </if>
		<if value="$ui['desc']">
		...
		<if value="$ui[bottom_ok]">
            ...
            <a class="more" href="{$m.url}" title="{$m.name}"  {$m.urlnew}  {$m.nofollow}>{$ui.arrow}</a>
        </tag>
	    </if>
	</div>
</div>