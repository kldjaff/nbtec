...
class met_auth extends admin {
	...
	public function __construct() {
		...
		$this->post = $_M['form'];
        include_once PATH_WEB.'templates'.DS.$_M['config']['met_skin_user'].DS.'metinfo.inc.php';
        $this->temptype = &$template_type;
		...
		(
			...
			'index_footer' => $this->getIndexFooter(),//模板底部版权
			...
			'met_agents_metmsg' => $this->met_agents_metmsg,//应用功能开启关闭
            'met_agents_set' => $this->met_agents_set,//高级设置功能开启关闭
	...
	public function updateConfig()
	{
		...
		foreach ($data as $key => $value) {
            ...
            if($key=="met_agents_index_footer"){
                $this->update($key,$this->post['index_footer']);
            }
		}
	}
	...
	public function replaceIndexFooter() {
        ...
        $newstr = '<div class="powered_by_metinfo">' . $this->post['index_footer'] . '</div>';
        $str = preg_replace('/<div class="powered_by_metinfo">([\s\S]*?)<\/div>/', $newstr, $str);
	...
	public function getIndexFooter()
	{
		...
		if(!$res[0]){
			preg_match('/<ui name=\"foot_info\" style=\"([^\"]+)\"/', $str, $out);
			if($out[1]){
				$this->path['index_footer'] = PATH_WEB.'templates'.DS.$_M['config']['met_skin_user'].DS.'ui'.DS.'foot_info'.DS.$out[1].DS.'index.php';
        ...
        if($this->temptype =='ui'){
            if(isset($_M['config']['met_agents_index_footer'])&&$_M['config']['met_agents_index_footer']==''){
                return $res[1];
            }else{
                return $_M['config']['met_agents_index_footer'];
            }
        }
    ...
    public function filter()
    {
        ...
        $data['met_agents_index_footer'] = htmlspecialchars_decode($this->post['index_footer']);
