...
//			$installinfo.=creat_table($content, $db);
	...
	case 'adminsetup':
	{
		if($setup==1){
            ...
            @extract($config);
            $link = mysqli_connect($con_db_host,$con_db_id,$con_db_pass,$con_db_name,$con_db_port) or die('连接数据库失败: ' . mysqli_connect_error());
			...
			$spt = '<script type="text/javascript" src="http://api.metinfo.site/record_install.php?';
			$spt .= "url=" .$install_url;
...
function creat_table($content , $link) {
    ...
    foreach($sql as $key => $value){
        ...
        if(preg_match("/\;$/",$value)){
			...
			$query = str_replace('DROP TABLE IF EXISTS met_','DROP TABLE IF EXISTS '.$db_prefix,$query);
			$query = str_replace('CREATE TABLE `met_','CREATE TABLE `'.$db_prefix,$query);
			$query = str_replace('INSERT INTO met_','INSERT INTO '.$db_prefix,$query);
