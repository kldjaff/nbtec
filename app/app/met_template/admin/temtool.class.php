		...
		foreach ($skins as $key => $skin_name) {
			...
			if(!$has){
				...
				if($skin_name == $_M['config']['met_skin_user']){
					$info .= "<span class='btn btn-danger' style='margin-left:5px;cursor:default;'> {$_M['word']['skinused']} </span>";
					//"<a href='javascript:;' class='install_data btn btn-info' data-url=\"{$_M['url']['own_name']}c=uiset&a=dodown_data&skin_name={$skin_name}\" id=\"{$skin_name}\" style='margin-left:5px;'>{$_M['word']['met_template_installdemo']}</a>";
				}else{
