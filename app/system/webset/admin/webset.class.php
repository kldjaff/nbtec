...
class webset extends admin {
	...
	function __construct() {
		...
		nav::set_nav(3, $_M['word']['third_party_code'], $_M['url']['own_form'].'a=dothirdparty');
	}
	...
	function doindex() {
		...
		if($_M['form']['turnovertext']){
			...
			$record = "http://api.metinfo.site/record_install.php?url={$_M['config']['met_weburl']}&email={$email}&webname={$_M['config']['met_webname']}&webkeywords={$_M['config']['met_keywords']}&tel={$tel}&version={$_M['config']['metcms_v']}&softtype=1";
		}
