...
$(function(){
	...
	$(document).on('hide.bs.modal', '.modal', function(event) {
		...
		if($iframe.length && $iframe.prop('contentWindow').location.href.indexOf('turnovertext')>=0){
			$page_iframe.prop('contentWindow').location.reload();
		}
		// 弹框隐藏时隐藏弹框中iframe中的弹框
		$('iframe',this).contents().find('.modal .close').click();
		// 隐藏图片添加组件弹框
		$('iframe',this).contents().find('.popover .outside-cancel').click();
    });
    // 弹窗中iframe返回上一页
	...
	$('.page-head-nav [data-url]').click(function(event) {
		...
		$nav_modal.modal().find('.modal-title').html($(this).attr('title'));
	});
	// 当前页面参数、风格设置参数弹框
	...
	$page_iframe.load(function() {
		...
		if(page_iframe_window.M){// 支持可视化的模板
	        ...
	        // 添加文字编辑按钮和区块设置组件
			var pageset_html="<link rel='stylesheet' type='text/css' href='"+M['weburl']+"app/system/include/static2/fonts/web-icons/web-icons.min.css'>"
		...
		// 右键菜单
		$('[met-imgmask]',page_iframe_document).contextMenu('#met-menu');
...
function editableSave(text,func){
	var $pageeditor_editor=$page_iframe.contents().find('.pageeditor-btn .pageeditor-editor'),
		$editable_click=$page_iframe.contents().find(".editable-click").eq($pageeditor_editor.attr('data-index'));
	$.ajax({
...
$.fn.pageinfo=function(){
    ...
    // 图片内容转换可视化信息
    $(this).find("img,[data-original]").each(function(index, el) {
    	var img_url=$(this).data('original')||$(this).data('lazy')||$(this).data('src')||$(this).attr('src');
    	if(img_url){
    		var imgval=img_url.indexOf('met-id')>=0?(img_url.indexOf('include/thumb.php?dir=')>=0?img_url.split('?')[2].split('&'):img_url.split('?')[1].split('&')):'';
    		if(imgval.length && imgval[0].indexOf('met-id')>=0) $(this).attr({'met-id':imgval[0].split('=')[1],'met-table':imgval[1].split('=')[1],'met-field':imgval[2].split('=')[1]});
    	}
    });
    // 图标内容转换可视化信息
    $(this).find("[class*='met-icon|']").each(function(index, el) {
...
$.fn.extend({
	contextMenu:function(menu_obj) {
		...
		if(!menu_obj) {
			menu_obj='#menu';
			$('body').append('<div id="menu"><ul></ul></div>');
		}
	...
	hrefPageset:function() {
		if($(this).attr('src') && $(this).attr('src').indexOf(M['weburl'])<0) return true;
		$(this).load(function() {
				...
				$(iframe_document).on('click','a[href!=""]',function(e){
					var url=$(this).attr('href'),
						...
						href_control=(function(){
							if($(this).attr('data-toggle')=='dropdown' && $(this).attr('data-hover')=='dropdown') return false;
							if(url=='#') return false;
							...
							if(url.indexOf(M['weburl']+'shop/')>=0) return false;
							if(url.indexOf(M['weburl']+'member/')>=0) return false;
							return true;
						})();
					if(href_control){
