...
class user_idvalid  {
    ...
    {
        global $_M;
        if (!preg_match("/^((13[0-9])|(14[5|7])|(15([0-3]|[5-9]))|(18[0,5-9]))\d{8}$/", $_M['form']['phone'])) return false; //手机号码不合法
        if (!preg_match("/(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/", $_M['form']['idcode'])) return false; //身份证号码不合法

        $data = array();
        $data['express_key']  = $_M['config']['met_idvalid_key'];
        $data['usersite']     = $this->url;
        $data['name']   = $_M['form']['realname'];
        $data['idcode'] = $_M['form']['idcode'];
        $data['phone']  = $_M['form']['phone'];

        #$url = "https://{$_M['config']['met_host_new']}/index.php?n=userver&c=idvalidate&a=dovalidate";
    ...
    public function checkBalance()
    {
        ...
        #$url = "http://app.test.metinfo.cn/index.php?n=userver&c=idvalidate&a=dogetbalance";
        #$url = "https://{$_M['config']['met_host_new']}/index.php?n=userver&c=idvalidate&a=dogetbalance";
    ...
    public function checkNumber(){
        ...
        #$url = "https://{$_M['config']['met_host_new']}/index.php?n=userver&c=idvalidate&a=docheck";
